  ...
  void back() {
    ...
    for (int a = 0; a < MAX_X; ++a) {
      ...
      for (int k = 0; k < 2; ++k) {
        ...
        del({a, b});
        auto next = [&](int m, int8_t *CT, int8_t *DT) {
          int c = a + m, ct = CT[type[c]];
          int d = b + m, dt = DT[type[d]];
          if (ct == -1 || dt == -1) return;
          int pct = type[c], pcc = color[c];
          int pdt = type[d], pdc = color[d];
          del({c, d});
          if (put(c, ct) && put(d, dt)) {
            auto value = [&]() {
              ll x = 0;
              auto f = [](int x) { return x < 1 ? 0 : x * x; };
              for (int i = 0; i < 6; ++i)
        ...
        if (pat == 3 && pbt == 2) {
          static int8_t DA[] = {-1, 4, -1, -1, -1, 3};
          static int8_t DB[] = {4, -1, -1, -1, -1, 2};
          next(-ROW, DA, DB);
        } else if (pat == 0 && pbt == 1) {
          static int8_t DA[] = {-1, -1, 4, -1, -1, 0};
          static int8_t DB[] = {-1, -1, -1, 4, -1, 1};
          next(ROW, DA, DB);
        } else if (pat == 1 && pbt == 2) {
          static int8_t DA[] = {-1, -1, -1, 5, 1, -1};
          static int8_t DB[] = {5, -1, -1, -1, 2, -1};
          next(-1, DA, DB);
        } else if (pat == 0 && pbt == 3) {
          static int8_t DA[] = {-1, -1, 5, -1, 0, -1};
          static int8_t DB[] = {-1, 5, -1, -1, 3, -1};
          next(1, DA, DB);
        }
        put(a, pat, pac);
        put(b, pbt, pbc);
    ...
    if (n.v > INT_LEAST64_MIN) neighbors(n);
  }
